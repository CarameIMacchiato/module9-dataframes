...
prev.salaries <- c(runif(100, 40000, 50000))
...
next.salaries <- c(prev.salaries + runif(100, -1000, 1000))

# Create a data.frame 'salaries' by combining the 3 vectors you just made
# Remember to set `stringsAsFactors=FALSE`!
salaries <- data.frame(employees, prev.salaries, next.salaries, stringsAsFactors = FALSE)

# Create a column 'raise' that stores the size of the raise between 2014 and 2015
salaries$raise <- next.salaries - prev.salaries
